...
public class BookDataCollectingJob implements DataCollectingJob {
  ...
  public void run(final JobConfiguration configuration) {
    ...
    final var books =
        searchEngine.search(title, categoryCode, runAt, configuration.getPeriod().getValue());
    repository.saveAll(
        configuration.getUsername(), configuration.getCategory(), runAt, period, title, books);
  }
